...
```python
ser = pd.Series(np.array([1, 2, 3]), ['a', 'b', 'c']) 
```
...
by index
```python
ser['a'] # first (returns scalar)
ser['c'] # last (returns scalar)
ser[['a']] # first
ser[['c']] # last
ser[['a','c']] # first and third
ser['b':] # everything but the first
ser[:'b'] # everything but the last
ser[:'a'] # first
ser['c':] # last
```
...
df = pd.DataFrame({'v1': ['x', 'y', 'z'],
                  'v2': [1, 2, 3]},
                 index=['a', 'b', 'c'])

# create from dict of series - each key is a column
df = pd.DataFrame({'v1': pd.Series(['x', 'y', 'z'], index=['a', 'b', 'c']),
...
df = pd.DataFrame([1, 2, 3], columns=['v1'])
...
```python
df.loc['a'] # first row - returns a series
df.loc["c"] # last row - returns a series
df.loc[["a"]] # first row
df.loc[["c"]] # last row
```
...
```python
df.loc[df['v1'].isin(['x', 'y'])]
```

#### subset columns by name {#subset-columns-by-name}

```python
df['v1'] # returns a series
df[['v1']]
df[['v1', 'v2']]
```
...
```python
df.columns[0] = 'c1' # TypeError: Index does not support mutable operations
df.columns = ['c1', 'c2', 'c3']
...
```python
df.index[0] = 'r1' # TypeError: Index does not support mutable operations
df.index = ['r1', 'r2', 'r3']
